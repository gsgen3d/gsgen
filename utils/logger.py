...
class SimplerHumanOutputFormat(HumanOutputFormat):
    def __init__(self, filename_or_file: Union[str, TextIO]):
        """
        log to a file, in a human readable format
    ...
    def write(self, key_values: Dict, key_excluded: Dict, step: int) -> None:
        ...
        for (key, value), (_, excluded) in zip(
            ...
            tag2str[tag].append(self._truncate(key))
            tag2str[tag].append(self._truncate(value_str))
        ...
        lines = []
        for tag, line in tag2str.items():
            lines.append(" ".join([tag] + line))
        self.file.write("\n".join(lines) + "\n")
...
def make_output_format(_format: str, log_dir: str, log_suffix: str = "") -> KVWriter:
    ...
    if _format == "stdout":
        ...
        return SimplerHumanOutputFormat(os.path.join(log_dir, f"log{log_suffix}.txt"))
    elif _format == "json":
